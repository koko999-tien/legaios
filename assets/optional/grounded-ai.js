...
function render(answer,rows,meta={}){
 ...
 const out=document.createElement('div');out.className='grounded-ai-output';out.textContent=String(answer||'');
...
async function ask(){
 ...
 try{
   const res=await fetch('/.netlify/functions/grounded-ai',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({question:q,sources:rows})});
