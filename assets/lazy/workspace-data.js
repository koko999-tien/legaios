...
const OFFICIAL_CANDIDATE_KEY="v15_official_candidates";
const OFFICIAL_CANDIDATE_DOMAINS=["vbpl.vn","vanban.chinhphu.vn","congbao.chinhphu.vn","chinhphu.vn","vbpl.moj.gov.vn"];
function officialCandidateUrlV15(v){try{const u=new URL(String(v||""));const h=u.hostname.toLowerCase().replace(/^www\./,"");if(u.protocol!=="https:"||!OFFICIAL_CANDIDATE_DOMAINS.some(d=>h===d||h.endsWith("."+d)))return "";u.hash="";return u.toString()}catch{return ""}}
function officialCandidateNormalizeV15(x){
  if(!x||typeof x!=="object")return null;const url=officialCandidateUrlV15(x.url);if(!url)return null;
...
function officialCandidateSaveV15(rows){STORE.set(OFFICIAL_CANDIDATE_KEY,rows.slice(0,200))}
function officialCandidateAddV15(raw){
  const row=officialCandidateNormalizeV15(raw);if(!row){toast("Nguồn web không hợp lệ");return false}
  const rows=officialCandidateRowsV15(),old=rows.find(x=>x.url===row.url);
  if(old){toast("Nguồn này đã có trong hàng rà soát");return false}
  row.profileId=typeof currentComplianceId==="string"?currentComplianceId:"";rows.unshift(row);officialCandidateSaveV15(rows);renderLawWatchV16();toast("Đã đưa nguồn web vào hàng rà soát");return true;
}
function officialCandidatePatchV15(id,patch){
  const rows=officialCandidateRowsV15(),x=rows.find(v=>v.id===id);if(!x)return;Object.assign(x,patch,{updatedAt:new Date().toISOString()});
  if(patch.status==="done"&&!x.reviewedAt)x.reviewedAt=new Date().toISOString();if(patch.status&&patch.status!=="done")x.reviewedAt="";officialCandidateSaveV15(rows);
}
function officialCandidateRemoveV15(id){const rows=officialCandidateRowsV15(),x=rows.find(v=>v.id===id);if(!x)return;if(confirm("Bỏ nguồn web này khỏi hàng rà soát?")){officialCandidateSaveV15(rows.filter(v=>v.id!==id));renderLawWatchV16();toast("Đã bỏ nguồn web khỏi hàng rà soát")}}
function officialCandidateHtmlV15(){
  const rows=officialCandidateRowsV15().sort((a,b)=>(a.status==="done")-(b.status==="done")||String(b.addedAt).localeCompare(String(a.addedAt)));
  if(!rows.length)return '<section class="candidate-review-section"><div class="candidate-review-head"><div><div class="section-kicker">SEARCH V4</div><h3>Nguồn web chờ rà soát</h3></div><small>Chưa có nguồn web nào được đưa vào hàng rà soát.</small></div></section>';
  const profiles=Array.isArray(complianceProfiles)?complianceProfiles:[];
  return '<section class="candidate-review-section"><div class="candidate-review-head"><div><div class="section-kicker">SEARCH V4</div><h3>Nguồn web chờ rà soát</h3></div><span class="candidate-review-badge">'+rows.length+' nguồn · chưa kiểm định</span></div><p class="candidate-review-note">Các mục dưới đây chỉ là liên kết phát hiện trên miền chính thức. Chưa được nhập vào kho pháp luật và chưa được coi là xác nhận hiệu lực hay phạm vi áp dụng.</p>'+rows.map(x=>{
    ...
    return '<article class="candidate-review-item" data-official-candidate="'+esc(x.id)+'"><div class="candidate-review-top"><div><div class="k">'+esc(x.host)+' · KẾT QUẢ WEB</div><h4>'+esc(x.title)+'</h4><small>'+esc(x.query?"Truy vấn: "+x.query:"Nguồn được thêm từ Search V4")+'</small></div><button class="tiny" data-official-candidate-remove="'+esc(x.id)+'" type="button">Bỏ</button></div><div class="g2 candidate-review-fields"><label class="field"><span>Trạng thái</span><select data-official-candidate-status="'+esc(x.id)+'"><option value="review" '+(x.status==="review"?"selected":"")+'>Cần rà</option><option value="active" '+(x.status==="active"?"selected":"")+'>Đang rà</option><option value="done" '+(x.status==="done"?"selected":"")+'>Đã rà thủ công</option></select></label><label class="field"><span>Hồ sơ tuân thủ</span><select data-official-candidate-profile="'+esc(x.id)+'">'+po+'</select></label><label class="field candidate-review-note-field"><span>Ghi chú rà soát</span><input data-official-candidate-note="'+esc(x.id)+'" value="'+esc(x.note)+'" placeholder="Hiệu lực, sửa đổi, điểm cần đối chiếu…"></label></div><div class="row candidate-review-actions"><a class="btn bs" href="'+esc(x.url)+'" target="_blank" rel="noopener noreferrer">Mở nguồn chính thức</a><span class="candidate-review-warning">Chưa nhập vào dữ liệu đã kiểm định</span></div></article>';
  }).join("")+'</section>';
}
