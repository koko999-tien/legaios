...
function officialCandidatePatchV15(id,p){const r=officialCandidateRowsV15(),x=r.find(v=>v.id===id);if(x){Object.assign(x,p);officialCandidateSaveV15(r)}}
function officialCandidateRemoveV15(id){const r=officialCandidateRowsV15();if(r.some(v=>v.id===id)&&confirm("Bỏ nguồn web này khỏi hàng rà soát?")){officialCandidateSaveV15(r.filter(v=>v.id!==id));renderLawWatchV16();toast("Đã bỏ nguồn web khỏi hàng rà soát")}}
function officialCandidateHtmlV15(){const r=officialCandidateRowsV15().sort((a,b)=>(a.status==="done")-(b.status==="done")||String(b.addedAt).localeCompare(String(a.addedAt)));if(!r.length)return'<section class="candidate-review-section"><div class="candidate-review-head"><div><div class="section-kicker">SEARCH V4</div><h3>Nguồn web chờ rà soát</h3></div><small>Chưa có nguồn web chờ rà.</small></div></section>';const p=Array.isArray(complianceProfiles)?complianceProfiles:[];return'<section class="candidate-review-section"><div class="candidate-review-head"><div><div class="section-kicker">SEARCH V4</div><h3>Nguồn web chờ rà soát</h3></div><span class="candidate-review-badge">'+r.length+' nguồn · chưa kiểm định</span></div><p class="candidate-review-note">Liên kết trên miền chính thức; chưa xác nhận hiệu lực hay phạm vi áp dụng.</p>'+r.map(x=>{const po='<option value="">Không gắn hồ sơ</option>'+p.map(v=>'<option value="'+esc(v.id)+'" '+(v.id===x.profileId?"selected":"")+'>'+esc(v.name||"Hồ sơ")+'</option>').join(""),host=new URL(x.url).hostname.replace(/^www\./,"");return'<article class="candidate-review-item"><div class="candidate-review-top"><div><div class="k">'+esc(host)+' · KẾT QUẢ WEB</div><h4>'+esc(x.title)+'</h4><small>'+esc(x.query?"Truy vấn: "+x.query:"Nguồn từ Search V4")+'</small></div><button class="tiny" data-official-candidate-remove="'+esc(x.id)+'" type="button">Bỏ</button></div><div class="g2 candidate-review-fields"><label class="field"><span>Trạng thái</span><select data-official-candidate-status="'+esc(x.id)+'"><option value="review" '+(x.status==="review"?"selected":"")+'>Cần rà</option><option value="active" '+(x.status==="active"?"selected":"")+'>Đang rà</option><option value="done" '+(x.status==="done"?"selected":"")+'>Đã rà thủ công</option></select></label><label class="field"><span>Hồ sơ tuân thủ</span><select data-official-candidate-profile="'+esc(x.id)+'">'+po+'</select></label><label class="field candidate-review-note-field"><span>Ghi chú</span><input data-official-candidate-note="'+esc(x.id)+'" value="'+esc(x.note)+'" placeholder="Hiệu lực, sửa đổi, điểm cần đối chiếu…"></label></div><div class="row candidate-review-actions"><a class="btn bs" href="'+esc(x.url)+'" target="_blank" rel="noopener noreferrer">Mở nguồn chính thức</a><span class="candidate-review-warning">Chưa vào dữ liệu đã kiểm định</span></div></article>'}).join("")+'</section>'}
...
function workspaceRestoreCountsV9(next){
  ...
  return {
    ...
    lifecycle:next.projectLifecycle&&typeof next.projectLifecycle==="object"?Object.keys(next.projectLifecycle).length:0
  };
}
function workspaceConfirmRestoreV9(next){
  ...
  return new Promise(resolve=>{
    ...
    [
      ...
      ["Căn cứ hồ sơ",counts.citations],["Văn bản theo dõi",counts.lawWatch],["Nguồn web chờ rà",counts.officialCandidates],["Vòng đời dự án",counts.lifecycle],["Tiến độ đọc",counts.reading]
...
function importWorkspaceV8(file){
  ...
  const r=new FileReader();r.onload=async()=>{try{
    ...
    next.readingProgress=d.readingProgress!==undefined?workspaceNormalizeReadingV8(d.readingProgress):readingProgressStore();next.citationBasket=d.citationBasket!==undefined?workspaceNormalizeCitationsV8(d.citationBasket):citationBasketV13;next.citationMemoMeta=d.citationMemoMeta!==undefined?{title:safeImportedText(d.citationMemoMeta?.title||"",500),note:safeImportedText(d.citationMemoMeta?.note||"",10000)}:citationMemoMetaV13;next.trash=Array.isArray(d.trash)?d.trash.slice(0,30):workspaceTrashRowsV8();next.projectLifecycle=window.LEGALOS_PROJECT_LIFECYCLE?.normalizeStore(d.projectLifecycle!==undefined?d.projectLifecycle:STORE.get("v17_project_lifecycle",{}))||{};
    ...
    const entries=[["w3_saved",next.saved],["w3_recent",next.recent],["w3_notes",next.notes],["w3_proc",next.procDone],["w3_cases",next.cases],["v10_expert_briefs",next.expertBriefs],[COMPLIANCE_KEY,next.complianceProfiles],[COMPLIANCE_AUDIT_KEY,next.complianceAudit],["v8_quick_note",next.quickNote],["v8_ui_prefs",next.uiPrefs],["v14_reading_progress",next.readingProgress],["v13_citation_basket",next.citationBasket],["v13_citation_meta",next.citationMemoMeta],[WORKSPACE_TRASH_KEY,next.trash],[LAW_WATCH_KEY,next.lawWatch],[OFFICIAL_CANDIDATE_KEY,next.officialCandidates],["v17_project_lifecycle",next.projectLifecycle]];
    ...
    if($("caseDetail")){$("caseDetail").className="empty";$("caseDetail").textContent="Chọn hồ sơ để xem chi tiết."}renderLawWatchV16();applyUIPrefs();syncQuickNote(quickNote);renderMemoV13();renderWorkspace();renderWorkspaceTrashV8();renderComplianceWorkspace();renderComplianceHome();window.LEGALOS_PROJECT_LIFECYCLE?.render();renderProcList();docs(curTopic(),$("q").value);toast("Đã khôi phục dữ liệu");
