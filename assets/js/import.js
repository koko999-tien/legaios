...
async function deleteImported(id){if(!confirm('Xóa tài liệu này khỏi hệ thống?'))return;await importDbDelete(id);if(selectedImportId===id){selectedImportId=null;if(importObjectURL){URL.revokeObjectURL(importObjectURL);importObjectURL=null}$('importDetail').innerHTML='<div class="import-preview-empty">Chọn một tài liệu để xem thông tin.</div>'}await refreshImportedDocs();toast('Đã xóa tài liệu')}
function exportImportIndex(){const data=importedDocs.map(({blob,...x})=>x),out={app:'Căn cứ Pháp lý Môi trường',schema:'ccplmt-document-index-v1',exportedAt:new Date().toISOString(),count:data.length,files:data};const b=new Blob([JSON.stringify(out,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='Can-cu-phap-ly-moi-truong-document-index.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500)}
